...
- Name: smoke.spec.js >> landing page renders for signed-out users
- Location: tests\smoke.spec.js:203:1
...
Locator: getByRole('heading', { name: /Automated Code Review Powered by AI/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: /Automated Code Review Powered by AI/i })
...
```ts
  105 |       github: { connected: true, username: 'octocat' },
  106 |       repo: { linked: true, linkedUsername: 'octocat' },
  107 |     });
  108 |   }
  109 | 
  110 |   if (path === '/settings/acme/gitguard') {
  111 |     return json({
  112 |       ok: true,
  113 |       settings: {
  114 |         enabled: true,
