...
async function upsertRepoSettings(req, res, next) {
  try {
    ...
    const settings = await repoSettingsService.updateRepoSettings({ owner, repo, update });
    res.json({ ok: true, settings });
  } catch (err) {
    next(err);
  }
...
async function getRepoHistory(req, res, next) {
  try {
    ...
    const history = await reviewRunService.getRepoHistory({ owner, repo, limit: Math.min(100, Math.max(1, limit)) });
...
async function getRepoInsights(req, res, next) {
  try {
    const { owner, repo } = req.params;
    const history = await reviewRunService.getRepoHistory({ owner, repo, limit: 50 });

    const completed = history.filter((h) => h.status === 'completed');
    const avgTotalMs = completed.length
      ? Number((completed.reduce((acc, run) => acc + (run.timingsMs?.total || 0), 0) / completed.length).toFixed(2))
      : 0;
    const avgRiskScore = completed.length
      ? Number((completed.reduce((acc, run) => acc + (run.avgRiskScore || 0), 0) / completed.length).toFixed(2))
      : 0;
